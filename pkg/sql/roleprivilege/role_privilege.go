...
// RolePrivilegeToSQLColumn is a map of roleprivilege (Kind) -> System.users SQL Column Name (string).
var RolePrivilegeToSQLColumn = map[Kind]string{
	CREATEROLE:   "hasCreateRole",
	NOCREATEROLE: "hasCreateRole",
}

// RolePrivilegeToBool is a map of roleprivilege (Kind) -> bool value in system.users table (bool)
var RolePrivilegeToBool = map[Kind]bool{
	CREATEROLE:   true,
	NOCREATEROLE: false,
}
...
func (pl List) CreateSetStmtFromRolePrivileges() (string, error) {
	if len(pl) <= 0 {
		return "", errors.New("no role privileges found")
	}
	setStmt := "SET "
	for i, privilege := range pl {
		if i == 0 {
			setStmt += fmt.Sprintf(
				"\"%s\" = %t ",
				RolePrivilegeToSQLColumn[privilege],
				RolePrivilegeToBool[privilege])
		} else {
			setStmt += fmt.Sprintf(
				", \"%s\" = %t ",
				RolePrivilegeToSQLColumn[privilege],
